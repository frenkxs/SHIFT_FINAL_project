...
# This script is indentical to '01_data_cleaning.R', the only difference is that in this we 
# only consider depressive syndromes and depressive disorders as diagnosis of depression. 
# Patients with depressive symptoms are considered depression free.
...
# We consider only DSM defined depression (depressive syndrome and depressive disorder)
...
dep_inc <- dep_rs3 %>%
  select(contains("CLEANtotalevent")) %>%
  mutate(across(everything(),~ as.character(.)))

dep_date <- dep_rs3 %>%
  select(contains("CLEANdate_totalevent")) 

dep_inc[dep_inc == "depressive symptoms" | dep_inc == "Depressive symptoms"] <- "no"

dep_inc[dep_inc == "no"] <- NA
dep_inc[!is.na(dep_inc)] <- 1
dep_inc[is.na(dep_inc)] <- 0
dep_date[dep_inc == 0] <- NA

# get those that were diagnosed with DSM depression after screening positive for depressive symptoms
temp <- dep_date %>%
  ...
  filter(first != "CLEANdate_totalevent2yr1")

dep_date <- dep_date %>%
  mutate(row = row_number())

#  lookup what round of examination it was and assign the right date to it
lookup_date <- function(row, column){
  dep_date[row, column]
}

temp$date <- as.Date(mapply(lookup_date, temp$row, temp$first), origin = "1970-01-01")

# finally, get the diagnosis date of DSM depression for all participants
dep_endd <- dep_date$CLEANdate_totalevent2yr1
dep_endd[dep_date$row %in% temp$row] <- temp$date

# depression first case
dep_rs3_primary <- dep_rs3[, c(1:6)] %>%
  add_column(eventd_dep1 = dep_endd) %>%
  mutate(inc_dep1 = ifelse(is.na(eventd_dep1), "no", "yes"))


# remove redundant columns
dep_rs3_primary <- dep_rs3_primary[, -c(2, 5, 6)]
...
dep_rs2_p <- "Datasets/Depression/ShortenedCohort_IncidenceDepressionRSII(03-2015).sav"
...
# depression first case
dep_rs2_primary <- dep_rs2[, c(1:6)] %>%
  add_column(eventd_dep1 = dep_endd) %>%
  mutate(inc_dep1 = ifelse(is.na(eventd_dep1), "no", "yes"))


# remove redundant columns
dep_rs2_primary <- dep_rs2_primary[, -c(4, 5, 6)]
...
dep_rs1_p <- "Datasets/Depression/Shortened_Cohort_IncidenceDepressionRSI(11-2013).sav"
...
# depression first case
dep_rs1_primary <- dep_rs1[, c(1:6)] %>%
  add_column(eventd_dep1 = dep_endd) %>%
  mutate(inc_dep1 = ifelse(is.na(eventd_dep1), "no", "yes"))


# remove redundant columns
dep_rs1_primary <- dep_rs1_primary[, -c(2, 5, 6)]
...
stroke_p <-  here::here("Datasets", "Stroke", "Strokes RSI-III 01-01-2016 (28-08-2017).sav")
...
tia_p <-  here::here("Datasets", "Stroke", "TIA (focal and mixed) RSI-III 01-01-2016 (28-08-2017).sav")
...
shift_data_dep <- list(start %>% select(ergoid, rs_cohort, fp_startdate, fp_censordate), 
...
shift_data_dep[] <- lapply(shift_data_dep, function(x) if(is.factor(x)) factor(x) else x)

# convert all character columns to factor
shift_data_dep[] <- lapply(shift_data_dep, function(x) if(is.character(x)) factor(x) else x)

# convert ergoid to factor
shift_data_dep$ergoid <- factor(shift_data_dep$ergoid)


save(shift_data_dep, file = here::here("Data", "Sensitivity-analysis", "shift_data_dep.Rdata"))  
